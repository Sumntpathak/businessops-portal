...
export function buildInstructions(session: CallSession): string {
  ...
  return [
    ...
    "- Latency masking is MANDATORY: BEFORE any lookup or checking the calendar, ALWAYS say ONE short natural filler in the caller's current language ('Ek second, main slots dekhti hoon…', 'Give me just a sec, pulling up the schedule…'), varying it every time — then call the tool immediately so there is zero silence.",
